...
def set_cabinet(
    ...
    **kwargs: Any,
    ...
    if document.cabinet and not replace:
        return None

    potential_cabinets = matching.match_cabinets(document, classifier)
    selected = potential_cabinets[0] if potential_cabinets else None
    if len(potential_cabinets) > 1 and not use_first:
        return None

    if (selected or replace) and not dry_run:
        logger.info(
            f"Assigning cabinet {selected} to {document}",
            extra={"group": logging_group},
        )
        document.cabinet = selected
        document.inherit_cabinet_permissions = selected is not None
        document.save(update_fields=("cabinet", "inherit_cabinet_permissions"))
