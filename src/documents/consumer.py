...
class ConsumerPlugin(
    ...
    def apply_overrides(self, document: Document) -> None:
        ...
        if self.metadata.cabinet_id:
            document.cabinet = Cabinet.objects.get(pk=self.metadata.cabinet_id)
            document.inherit_cabinet_permissions = (
                self.metadata.inherit_cabinet_permissions
                if self.metadata.inherit_cabinet_permissions is not None
                else True
            )
