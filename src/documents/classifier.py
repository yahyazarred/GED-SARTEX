...
class DocumentClassifier:
    ...
    def train(
        ...
        docs_queryset = (
            ...
            .select_related("document_type", "correspondent", "storage_path", "cabinet")
        ...
        for doc in docs_queryset:
            ...
            y = -1
            cabinet = doc.cabinet
            if cabinet and cabinet.matching_algorithm == MatchingModel.MATCH_AUTO:
                y = cabinet.pk
            hasher.update(y.to_bytes(4, "little", signed=True))
            labels_cabinet.append(y)
        ...
        num_cabinets: int = len(set(labels_cabinet) | {-1}) - 1
        ...
        if num_cabinets > 0:
            logger.debug("Training cabinet classifier...")
            notify(f"Training cabinet classifier ({num_cabinets} cabinet(s))...")
            self.cabinet_classifier = MLPClassifier(tol=0.01)
            self.cabinet_classifier.fit(data_vectorized, labels_cabinet)
        else:
            self.cabinet_classifier = None
            logger.debug("There are no cabinets. Not training cabinet classifier.")
    ...
    def predict_cabinet(self, content: str) -> int | None:
        if self.cabinet_classifier:
            X = self._vectorize(content)
            cabinet_id = self.cabinet_classifier.predict(X)
            if cabinet_id != -1:
                return cabinet_id
        return None
