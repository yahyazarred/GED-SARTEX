    ...
    def has_object_permission(self, request, view, obj):
        if (
            ...
            if action is not None:
                cabinet = obj.cabinet
                if cabinet.owner is None or cabinet.owner == request.user:
                    return True
                return request.user.has_perm(f"documents.{action}_cabinet", cabinet)
...
def permitted_document_ids(user, action: str = "view"):
    """
    ...
    perm_filter = {
        "permission__codename": f"{action}_document",
    ...
    cabinet_perm_filter = {
        "permission__codename": f"{action}_cabinet",
        "permission__content_type": cabinet_ct,
    }
    user_perm_cabinets = (
        UserObjectPermission.objects.filter(user=user, **cabinet_perm_filter)
        .annotate(object_pk_int=Cast("object_pk", IntegerField()))
        .values_list("object_pk_int", flat=True)
    )
    group_perm_cabinets = (
        GroupObjectPermission.objects.filter(group__user=user, **cabinet_perm_filter)
        .annotate(object_pk_int=Cast("object_pk", IntegerField()))
        .values_list("object_pk_int", flat=True)
    )
    permitted_cabinets = user_perm_cabinets.union(group_perm_cabinets)

    direct_document_access = (
        Q(owner=user)
        | Q(owner__isnull=True)
        | Q(
            id__in=permitted_documents,
        )
    )
    inherited_cabinet_access = (
        Q(cabinet__owner=user)
        | Q(cabinet__owner__isnull=True)
        | Q(cabinet_id__in=permitted_cabinets)
    )

    return base_docs.filter(
        (Q(cabinet__isnull=True) | Q(inherit_cabinet_permissions=False))
        & direct_document_access
        | Q(cabinet__isnull=False, inherit_cabinet_permissions=True)
        & inherited_cabinet_access,
...
def get_objects_for_user_owner_aware(
    ...
    *,
    ...
    normalized_perms = [perms] if isinstance(perms, str) else perms
    if Model is Document and len(normalized_perms) == 1:
        action = normalized_perms[0].split(".")[-1].removesuffix("_document")
        if action in {"view", "change"}:
            return manager.filter(pk__in=permitted_document_ids(user, action))
...
def has_perms_owner_aware(user, perms, obj):
    if (
        ...
        action = perms.split(".")[-1].removesuffix("_document")
        if action in {"view", "change"}:
            cabinet = obj.cabinet
            return (
                cabinet.owner is None
                or cabinet.owner == user
                or user.has_perm(f"documents.{action}_cabinet", cabinet)
            )
