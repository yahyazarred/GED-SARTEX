...
def _build_suggestion_table(
    ...
    for suggestion in suggestions:
        ...
        table.add_row(
            ...
            str(suggestion.cabinet) if suggestion.cabinet else "-",
        )
...
def _build_summary_table(stats: RetaggerStats) -> Table:
    ...
    table.add_row("Cabinets set", str(stats.cabinets))
...
class Command(PaperlessCommand):
    ...
    def add_arguments(self, parser) -> None:
        ...
        parser.add_argument("-C", "--cabinet", default=False, action="store_true")
    ...
    def handle(self, *args, **options) -> None:
        ...
        do_cabinet: bool = options["cabinet"]

        if not any(
            [do_correspondent, do_document_type, do_tags, do_storage_path, do_cabinet],
        ):
            self.console.print(
                "[yellow]No classifier targets specified. "
                "Use -c, -T, -t, -s, or -C to select what to retag.[/yellow]",
            )
            return
