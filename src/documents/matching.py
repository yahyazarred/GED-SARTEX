...
def match_cabinets(document: Document, classifier: DocumentClassifier, user=None):
    pred_id = (
        classifier.predict_cabinet(document.suggestion_content) if classifier else None
    )
    ...
    if user is not None:
        cabinets = get_objects_for_user_owner_aware(
            user,
            "documents.view_cabinet",
            Cabinet,
        )
    else:
...
def existing_document_matches_workflow(
    ...
    allowed_cabinet_ids = set(
        trigger.filter_has_any_cabinets.values_list("id", flat=True),
    )
    if allowed_cabinet_ids and document.cabinet_id not in allowed_cabinet_ids:
        return (
            False,
            f"Document cabinet {document.cabinet} is not one of {list(trigger.filter_has_any_cabinets.all())}",
        )

    if (
        trigger.filter_has_cabinet_id is not None
        and document.cabinet_id != trigger.filter_has_cabinet_id
    ):
        return (
            False,
            f"Document cabinet {document.cabinet} does not match {trigger.filter_has_cabinet}",
        )

    if (
        document.cabinet_id
        and trigger.filter_has_not_cabinets.filter(id=document.cabinet_id).exists()
    ):
        return (
            False,
            f"Document cabinet {document.cabinet} is excluded by {list(trigger.filter_has_not_cabinets.all())}",
        )
...
def prefilter_documents_by_workflowtrigger(
    ...
    if trigger.filter_has_any_cabinets.exists():
        documents = documents.filter(cabinet__in=trigger.filter_has_any_cabinets.all())
    if trigger.filter_has_cabinet is not None:
        documents = documents.filter(cabinet=trigger.filter_has_cabinet)
    if trigger.filter_has_not_cabinets.exists():
        documents = documents.exclude(cabinet__in=trigger.filter_has_not_cabinets.all())
