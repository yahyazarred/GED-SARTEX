...
def set_cabinet(
    ...
    *,
    ...
    if cabinet:
        cabinet = Cabinet.objects.only("pk").get(id=cabinet)

    target_inheritance = inherit_permissions if cabinet else False
    qs = Document.objects.filter(id__in=doc_ids).exclude(
        cabinet=cabinet,
        inherit_cabinet_permissions=target_inheritance,
    )
    affected_docs = list(qs.values_list("pk", flat=True))
    qs.update(
        cabinet=cabinet,
