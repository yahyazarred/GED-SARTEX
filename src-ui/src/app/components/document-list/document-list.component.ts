...
import { FILTER_CABINET, FILTER_FULLTEXT_MORELIKE, FILTER_HAS_CABINET_ANY } from 'src/app/data/filter-rule-type'
...
import { CabinetNamePipe } from 'src/app/pipes/cabinet-name.pipe'
  ...
  implements OnInit, OnDestroy
{
  ...
  get activeCabinet(): Cabinet | null {
    const rule = this.list.filterRules.find((item) =>
      [FILTER_CABINET, FILTER_HAS_CABINET_ANY].includes(item.rule_type)
    )
    const id = Number(rule?.value?.toString().split(',')[0])
    return this.cabinets().find((cabinet) => cabinet.id === id) ?? null
  }
  ...
  clickCabinet(cabinetId: number) {
    this.filterEditor.toggleCabinet(cabinetId)
    this.filterEditor.updateRules()
  }
