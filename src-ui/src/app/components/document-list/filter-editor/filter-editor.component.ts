  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  generateFilterName() {
    if (this.filterRules.length == 1) {
      ...
      switch (rule.rule_type) {
        ...
        case FILTER_CABINET:
        case FILTER_HAS_CABINET_ANY:
          if (rule.value) {
            return $localize`Cabinet: ${
              this.cabinetSelectionModel.items.find((c) => c.id == +rule.value)
                ?.name
            }`
          } else {
            return $localize`Without cabinet`
          }
  ...
  cabinetDocumentCounts: SelectionDataItem[]
  ...
  cabinetSelectionModel = new FilterableDropdownSelectionModel()
  ...
  set filterRules(value: FilterRule[]) {
    ...
    this.cabinetSelectionModel.clear(false)
    ...
    value.forEach((rule) => {
      switch (rule.rule_type) {
        ...
        case FILTER_DOES_NOT_HAVE_STORAGE_PATH:
          ...
          break
        case FILTER_CABINET:
          this.cabinetSelectionModel.intersection =
            rule.value == NEGATIVE_NULL_FILTER_VALUE.toString()
              ? Intersection.Exclude
              : Intersection.Include
          this.cabinetSelectionModel.set(
            rule.value ? +rule.value : null,
            this.cabinetSelectionModel.intersection == Intersection.Include
              ? ToggleableItemState.Selected
              : ToggleableItemState.Excluded,
            false
          )
          break
        case FILTER_HAS_CABINET_ANY:
          this.cabinetSelectionModel.logicalOperator = LogicalOperator.Or
          this.cabinetSelectionModel.intersection = Intersection.Include
          this.cabinetSelectionModel.set(
            rule.value ? +rule.value : null,
            ToggleableItemState.Selected,
          ...
          break
        case FILTER_DOES_NOT_HAVE_CABINET:
          this.cabinetSelectionModel.intersection = Intersection.Exclude
          this.cabinetSelectionModel.set(
            rule.value ? +rule.value : null,
            ToggleableItemState.Excluded,
            false
  ...
  get filterRules(): FilterRule[] {
    ...
    if (
      this.cabinetSelectionModel.isNoneSelected() &&
      this.cabinetSelectionModel.intersection == Intersection.Include
    ) {
      filterRules.push({ rule_type: FILTER_CABINET, value: null })
    } else {
      if (
        this.cabinetSelectionModel.isNoneSelected() &&
        this.cabinetSelectionModel.intersection == Intersection.Exclude
      ...
      this.cabinetSelectionModel.getSelectedItems().forEach((cabinet) => {
        filterRules.push({
          rule_type: FILTER_HAS_CABINET_ANY,
          value: cabinet.id?.toString(),
        })
      })
      this.cabinetSelectionModel
        .getExcludedItems()
        .filter((cabinet) => cabinet.id > 0)
        .forEach((cabinet) => {
          filterRules.push({
            rule_type: FILTER_DOES_NOT_HAVE_CABINET,
            value: cabinet.id?.toString(),
          })
        })
    }
  ...
  set selectionData(selectionData: SelectionData) {
    ...
    this.cabinetDocumentCounts = selectionData?.selected_cabinets ?? null
  ...
  ngOnInit() {
    ...
    if (
      ...
      this.loadingCountTotal++
      this.cabinetService.listAll().subscribe((result) => {
        this.cabinetSelectionModel.items = result.results
        this.maybeCompleteLoading()
      })
    }
    if (
  ...
  toggleCabinet(cabinetId: number) {
    this.cabinetSelectionModel.toggle(cabinetId)
  }
  ...
  onCabinetDropdownOpen() {
    this.cabinetSelectionModel.apply()
  }
