...
export enum TriggerFilterType {
  ...
  CabinetAny = 'cabinet_any',
  CabinetIs = 'cabinet_is',
  CabinetNot = 'cabinet_not',
...
interface TriggerFilterDefinition {
  ...
  selectItems?: 'correspondents' | 'documentTypes' | 'storagePaths' | 'cabinets'
  disabled?: boolean
}
...
type TriggerFilterAggregate = {
  ...
  filter_has_any_cabinets: number[]
  filter_has_not_cabinets: number[]
  filter_has_cabinet: number | null
...
const TRIGGER_FILTER_DEFINITIONS: TriggerFilterDefinition[] = [
  ...
  {
    id: TriggerFilterType.CabinetIs,
    name: $localize`Belongs to cabinet`,
    inputType: 'select',
    allowMultipleEntries: false,
    allowMultipleValues: false,
    selectItems: 'cabinets',
  },
  {
    id: TriggerFilterType.CabinetAny,
    name: $localize`Belongs to any of these cabinets`,
    inputType: 'select',
    allowMultipleEntries: false,
    allowMultipleValues: true,
    selectItems: 'cabinets',
  ...
  {
    id: TriggerFilterType.CabinetNot,
    name: $localize`Does not belong to these cabinets`,
    inputType: 'select',
    allowMultipleEntries: false,
    allowMultipleValues: true,
    selectItems: 'cabinets',
...
const FILTER_HANDLERS: Record<TriggerFilterType, FilterHandler> = {
  ...
  [TriggerFilterType.CabinetIs]: {
    apply: (aggregate, values) => {
      aggregate.filter_has_cabinet = Array.isArray(values)
        ? (values[0] ?? null)
        : values
    },
    extract: (trigger) => trigger.filter_has_cabinet,
    hasValue: (value) => value !== null && value !== undefined,
  },
  [TriggerFilterType.CabinetAny]: {
    apply: (aggregate, values) => {
      aggregate.filter_has_any_cabinets = Array.isArray(values) ? [...values] : [values]
    },
    extract: (trigger) => trigger.filter_has_any_cabinets,
    hasValue: (value) => Array.isArray(value) && value.length > 0,
  },
  [TriggerFilterType.CabinetNot]: {
    apply: (aggregate, values) => {
      aggregate.filter_has_not_cabinets = Array.isArray(values) ? [...values] : [values]
    },
    extract: (trigger) => trigger.filter_has_not_cabinets,
    hasValue: (value) => Array.isArray(value) && value.length > 0,
  },
}
  ...
  implements OnInit
{
  ...
  readonly cabinets = toSignal(
    this.cabinetService.listAll().pipe(map((result) => result.results)),
    { initialValue: undefined as Cabinet[] }
  )
    ...
    if (formValues?.triggers?.length) {
      formValues.triggers = formValues.triggers.map(
        (trigger: any, index: number) => {
          ...
          trigger.filter_has_any_cabinets = aggregate.filter_has_any_cabinets
          trigger.filter_has_not_cabinets = aggregate.filter_has_not_cabinets
          trigger.filter_has_cabinet = aggregate.filter_has_cabinet ?? null
  ...
  isMultiValueFilter(type: TriggerFilterType): boolean {
    switch (type) {
      ...
      case TriggerFilterType.CabinetAny:
      case TriggerFilterType.CabinetNot:
        return true
      default:
        return false
    }
  ...
  private createActionField(
    ...
    this.actionFields.push(
      new FormGroup({
        ...
        assign_cabinet: new FormControl(action.assign_cabinet),
        assign_inherit_cabinet_permissions: new FormControl(
          action.assign_inherit_cabinet_permissions ?? true
        ),
        ...
        remove_cabinets: new FormControl(action.remove_cabinets),
        remove_all_cabinets: new FormControl(action.remove_all_cabinets),
  ...
  addAction() {
    ...
    const action: WorkflowAction = {
      ...
      assign_cabinet: null,
      assign_inherit_cabinet_permissions: true,
      assign_owner: null,
      assign_view_users: [],
      assign_view_groups: [],
      assign_change_users: [],
